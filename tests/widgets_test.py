...
class TestWidgets:
    ...
    class TestAccordianPageAccordianPage:
    ...
    class TestAutoCompletePage:
        ...
        @allure.title('Check deletions of all elements from the multi autocomplete')
        def test_remove_all_value_from_multi(self, driver):
            autocomplete_page = AutoCompletePage(driver, 'https://demoqa.com/auto-complete')
            autocomplete_page.open()
            autocomplete_page.fill_input_multi()
            count_value_after = autocomplete_page.remove_all_value_from_multi()
            assert count_value_after == 0, 'Value was not deleted'

        @allure.title('Check autocomplete in the single form')
